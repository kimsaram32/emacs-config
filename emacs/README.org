...
#+begin_src elisp
  ...
  (defconst me:emacs-load-directory
    (file-name-concat me:dotfiles-directory "emacs/lisp")
...
** Files
#+begin_src elisp
  (defconst me:readme-org-file-name
    (expand-file-name "emacs/README.org" me:dotfiles-directory)
    "File name for README.org configuration file.")
#+end_src
* Load
#+begin_src elisp
  (add-to-list 'load-path me:emacs-load-directory)
#+end_src
...
#+end_src
** Finish today
I finish each day by committing changes I made in my dotfiles and personal files into the Git repository.
...
#+begin_src elisp
  ...
  (keymap-set prog-mode-map "C-c f" #'format-all-region-or-buffer)
#+end_src

Enable in html-ts-mode, because it's a text-mode.
#+begin_src elisp
  (with-eval-after-load 'html-ts-mode
    (keymap-set html-ts-mode-map "C-c C-f" #'format-all-region-or-buffer)
    (keymap-set html-ts-mode-map "C-c f" #'format-all-region-or-buffer))
#+end_src
** Languages
*** CSS
#+begin_src elisp
...
#+begin_src elisp
  (defun me:combobulate-display-tree-at-point ()
...
#+end_src
** Jump to heading
Helper function to get all headings in a hierarchical format.
#+begin_src elisp
  (defun me:org-headings-hierarhcy-list (buffer)
    "Return a list of all headings in org mode buffer BUFFER."
    (with-current-buffer buffer
      (org-map-entries
       (lambda ()
         (cons
...
#+begin_src elisp
  (defun me:org-jump-to-heading ()
    "Ask for a heading in the current buffer, and jump to there."
    (interactive)
    (let* ((candidates
            (me:org-headings-hierarhcy-list (current-buffer)))
           (target
            (map-elt candidates
                     (completing-read "Jump to headline: "
                                      candidates
                                      nil 'require-match))))
      (goto-char target)
  ...
  (keymap-set org-mode-map "C-c C-j" #'me:org-jump-to-heading)
#+end_src

Quickly jump to =README.org= with a heading.
#+begin_src elisp
  (defun me:jump-to-readme-org ()
    "Jump to a heading inside README.org."
    (interactive)
    (let* ((buf (or (get-file-buffer me:readme-org-file-name)
                    (find-file-noselect me:readme-org-file-name)))
           (candidates
            (me:org-headings-hierarhcy-list buf))
           (target
            (map-elt candidates
                     (completing-read "Jump to headline: "
                                      candidates
  ...
  (keymap-global-set "C-c r" #'me:jump-to-readme-org)
#+end_src
** todo
#+begin_src elisp
...
#+begin_src elisp
  (defvar me:elfeed-to-read-tag 'to-read-list
...
each entry, I hit =a= to add it, and =s= to skip it. When I'm done, I
...
#+begin_src elisp
  ...
  (keymap-set elfeed-show-mode-map "f" #'me:elfeed-show-skip)
#+end_src
*** Export the entries with =l=
Currently, exporting is done by pushing the list to the mark ring. I
...
In search mode:
#+begin_src elisp
  (defun me:elfeed-search-read ()
    "Read elfeed entries in search mode."
    (interactive)
    (let ((entries (elfeed-search-selected)))
      (elfeed-untag entries 'unread)
      (elfeed-untag entries me:elfeed-to-read-tag)
      (mapc #'elfeed-search-update-entry entries)
      (unless (or elfeed-search-remain-on-entry (use-region-p))
        (forward-line))))

  (keymap-set elfeed-search-mode-map "r" #'me:elfeed-search-read)
#+end_src

In show mode:
#+begin_src elisp
  (defun me:elfeed-show-read ()
    "Read current elfeed entry."
    (interactive)
    (elfeed-show-untag 'unread 'to-read-list))

  (keymap-set elfeed-show-mode-map "r" #'me:elfeed-show-read)
#+end_src
** Load entries
#+begin_src elisp
