...
My workflow is the following: I go through the unread entries, and for
each entry, I hit =a= to add it, and =s= to skip. When I'm done, I
export the entries to clipboard, paste them to Notes app, and read
them on my iPhone.
...
#+begin_src elisp
  (defun me:elfeed-show-add-to-read ()
    ...
    (when-let ((entry elfeed-show-entry))
      ...
      (elfeed-show-tag 'unread 'to-read-list)
...
#+begin_src elisp
  (defvar me:vms
    '((:name "ARM Ubuntu" :ssh-hostname "vm-ubuntu" :utm-name "Linux")
      (:name "x64 Debian" :ssh-hostname "vm-x64" :utm-name "Emulated Debian"))
    "List of VMs.
  ...
  ssh-hostname  The hostname for SSHing into the machine.
  utm-name      The name of the VM in UTM.")
#+end_src
** UTM integration
#+begin_src elisp
  (defun me:vm-start-all ()
    "Start all VMs."
    (interactive)
    (dolist (vm me:vms)
      (shell-command (format "utmctl start \"%s\"" (plist-get vm :utm-name))))
    (message (format "Successfully started %d VMs." (length me:vms))))

  (defun me:vm-stop-all ()
    "Stop all VMs."
    (interactive)
    (dolist (vm me:vms)
      (shell-command (format "utmctl stop \"%s\"" (plist-get vm :utm-name))))
    (message (format "Successfully stopped %d VMs." (length me:vms))))
#+end_src
** Manual program
#+begin_src elisp
  (defvar me:man-manual-programs
    '("ssh vm-ubuntu man"
      "ssh vm-x64 man"
      "man"))

  (defun me:man-change-program ()
    "Change the program to use in man."
    (interactive)
    (setq-default manual-program (completing-read "Manual program: "
                                                  me:manual-programs))
    (message (format "Manual program is now `%s'" manual-program)))

  (keymap-global-set "C-c v m" #'me:man-change-program)
#+end_src
* Markdown-mode
#+begin_src elisp
